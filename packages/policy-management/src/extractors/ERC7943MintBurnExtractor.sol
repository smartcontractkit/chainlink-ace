...
/**
 * @title ERC7943MintBurnExtractor
 * @notice Extracts parameters from ERC-7943 Compliance Token mint and burn function calls.
 * @dev This extractor supports mint(), burn(), and burnFrom() function selectors from the
 *      ComplianceTokenERC7943 contract and extracts the account and amount parameters.
...
contract ERC7943MintBurnExtractor is IExtractor {
  ...
  /**
   * @notice Extracts parameters from ComplianceTokenERC7943 mint, burn, and burnFrom function calls.
   * @dev Handles three function signatures:
   *      - mint(address to, uint256 amount): account and amount from calldata
   *      - burnFrom(address from, uint256 amount): account and amount from calldata
    ...
    returns (IPolicyEngine.Parameter[] memory)
  {
    ...
    // Handle mint(address to, uint256 amount) and burnFrom(address from, uint256 amount)
    if (
      payload.selector == ComplianceTokenERC7943.mint.selector
        || payload.selector == ComplianceTokenERC7943.burnFrom.selector
    ) {
      (account, amount) = abi.decode(payload.data, (address, uint256));
      // Handle burn(uint256 amount) - account is the sender
    } else if (payload.selector == ComplianceTokenERC7943.burn.selector) {
