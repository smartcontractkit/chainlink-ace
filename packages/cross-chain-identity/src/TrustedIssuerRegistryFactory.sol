...
contract TrustedIssuerRegistryFactory {
  ...
  /// @notice Raised when the trusted issuer registry unexpectedly already exists
  error TrustedIssuerRegistryAlreadyExists();
  ...
  /**
   * @notice Creates a new trusted issuer registry contract using deterministic minimal proxy cloning, reverting if the
  ...
  {
    return _createTrustedIssuerRegistry(implementation, uniqueRegistryId, policyEngine, initialOwner, false);
  }

  /**
   * @notice Creates a new trusted issuer registry contract using deterministic minimal proxy cloning, returning the
   *         existing address if the registry already exists.
   * @param implementation The address of the trusted issuer registry implementation contract to clone
   * @param uniqueRegistryId A unique identifier for this registry (combined with msg.sender to create salt)
   * @param policyEngine The address of the policy engine that will manage this registry
   ...
   */
  function getOrCreateTrustedIssuerRegistry(
    address implementation,
    bytes32 uniqueRegistryId,
    address policyEngine,
    address initialOwner
    ...
    returns (address registryAddress)
  {
    return _createTrustedIssuerRegistry(implementation, uniqueRegistryId, policyEngine, initialOwner, true);
  }

  /**
   * @dev Creates a new trusted issuer registry contract using deterministic minimal proxy cloning.
   ...
   */
  function _createTrustedIssuerRegistry(
    address implementation,
    bytes32 uniqueRegistryId,
    address policyEngine,
    address initialOwner,
    ...
    returns (address registryAddress)
  {
    ...
    if (registryAddress.code.length > 0) {
      if (idempotent) {
        ...
        revert TrustedIssuerRegistryAlreadyExists();
      }
    }
  ...
  /**
   * @notice  Creates a new trusted registry contract using an upgradeable deterministic proxy, reverting if the
   *          existing address if the registry already exists.
   * @param implementation The address of the trusted issuer registry implementation contract to clone
   * @param uniqueRegistryId A unique identifier for this registry (combined with msg.sender to create salt)
   * @param policyEngine The address of the policy engine that will manage this registry
   ...
   * @return registryAddress The address of the created registry contract
   */
  function createUpgradeableTrustedIssuerRegistry(
    address implementation,
    bytes32 uniqueRegistryId,
    address policyEngine,
    address initialOwner
    ...
    returns (address registryAddress)
  {
    return _createUpgradeableTrustedIssuerRegistry(implementation, uniqueRegistryId, policyEngine, initialOwner, false);
  }

  /**
   * @notice  Creates a new trusted registry contract using an upgradeable deterministic proxy, returning the
   *          existing address if the registry already exists.
   * @param implementation The address of the trusted issuer registry implementation contract to clone
   * @param uniqueRegistryId A unique identifier for this registry (combined with msg.sender to create salt)
   * @param policyEngine The address of the policy engine that will manage this registry
   ...
   * @return registryAddress The address of the created (or existing) registry contract
   */
  function getOrCreateUpgradeableTrustedIssuerRegistry(
    address implementation,
    bytes32 uniqueRegistryId,
    address policyEngine,
    address initialOwner
    ...
    returns (address registryAddress)
  {
    return _createUpgradeableTrustedIssuerRegistry(implementation, uniqueRegistryId, policyEngine, initialOwner, true);
  }

  /**
   * @dev Creates a new trusted issuer registry contract using an upgradeable deterministic proxy.
   ...
   *         idempotent is true - return the existing address.
   */
  function _createUpgradeableTrustedIssuerRegistry(
    address implementation,
    bytes32 uniqueRegistryId,
    address policyEngine,
    address initialOwner,
    ...
    returns (address registryAddress)
  {
    ...
    bytes memory initData =
      abi.encodeWithSelector(TrustedIssuerRegistry.initialize.selector, policyEngine, initialOwner);
  ...
  /**
   * @notice Predicts the deterministic address where a trusted issuer registry would be deployed using an upgradeable
   * proxy.
   * @dev Useful for calculating registry addresses before deployment or checking if a registry already exists.
   *      Uses the same salt generation as createUpgradeableTrustedIssuerRegistry to ensure address consistency.
   * @param creator The address of the account that would create the registry
   * @param implementation The address of the trusted issuer registry implementation contract
   * @param uniqueRegistryId The unique identifier for the registry
   * @param policyEngine The address of the policy engine that will manage this registry
